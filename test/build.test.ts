...
test('BUILD는 Tick별 참여와 중단을 기록한다', () => {
  ...
  world.agents[0].position = workPositions({ ...building, position })[0]
  storage.resources.Wood = building.inputs.Wood
  storage.resources.Stone = building.inputs.Stone

  const started = resolveTick(world, [{ agentId: 1, type: 'BUILD', buildingId: building.id, position, storageId: storage.id }])
  assert.deepEqual(started.buildings.find((item) => item.id === building.id).position, position)
