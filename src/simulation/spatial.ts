import { DIRECTIONS, SIMULATION_CONFIG } from '../config/simulation.ts'
...
export function blockedCells(world) {
  const placedBuildings = world.buildings.filter((building) => building.position)
  return new Set([
    ...footprintCells(world.camp),
    ...world.nodes.flatMap(footprintCells),
    ...placedBuildings.flatMap(footprintCells),
  ].map(keyOf))
}

export function buildingPlacementError(world, building, position) {
  if (!Number.isInteger(position?.x) || !Number.isInteger(position?.y)) return '최초 BUILD에는 정수 좌표가 필요합니다.'
  const candidate = { ...building, position }
  const footprint = footprintCells(candidate)
  if (footprint.some((cell) => !inside(cell))) return `${building.name} 배치가 맵 경계를 벗어납니다.`
  const occupied = blockedCells(world)
  if (footprint.some((cell) => occupied.has(keyOf(cell)))) return `${building.name} 배치가 기존 오브젝트와 겹칩니다.`
  if (!workPositions(candidate, occupied).length) return `${building.name}에 접근 가능한 작업 셀이 없습니다.`
  return null
}
