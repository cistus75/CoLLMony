...
import { footprintCells, keyOf, workPositions } from './spatial.ts'
...
function assertResourceMap(resources, resourceIds, label) {
  Object.entries(resources).forEach(([resource, amount]) => {
    ...
    assertConfig(Number.isFinite(Number(amount)) && Number(amount) > 0, `${label}의 ${resource} 수량은 0보다 커야 합니다.`)
  })
}
...
export function validateSimulationConfig() {
  ...
  assertUnique(RESOURCE_NODE_TYPES, '자원 노드')
  ...
  RESOURCE_NODE_TYPES.forEach((node) => {
  ...
  Object.entries(RESOURCE_CONFIG).forEach(([resource, meta]) => {
    if (meta.kind === 'survival' || meta.kind === 'raw') {
      assertConfig(RESOURCE_NODE_TYPES.some((node) => node.resource === resource), `${resource} 자원 노드가 없습니다.`)
    }
  })

  BUILDING_BLUEPRINTS.forEach((building) => {
    const { width, height } = building.size
    assertConfig(Number.isInteger(width) && width > 0 && Number.isInteger(height) && height > 0, `${building.name}의 크기는 양의 정수여야 합니다.`)
  ...
  const obtainable = new Set(RESOURCE_NODE_TYPES.map((node) => node.resource))
  ...
  const entities = [CAMP_CONFIG]
  assertConfig(CAMP_CONFIG.position.x === Math.floor((SIMULATION_CONFIG.width - CAMP_CONFIG.size.width) / 2)
    && CAMP_CONFIG.position.y === Math.floor((SIMULATION_CONFIG.height - CAMP_CONFIG.size.height) / 2), '캠프는 World 중앙에 고정되어야 합니다.')
