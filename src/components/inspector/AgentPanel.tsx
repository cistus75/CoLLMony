...
import { TRAIT_LABELS } from '../../config/ui.ts'
  ...
  return (
    <section className={cn(className)}>
      ...
      <div className="grid grid-cols-[1fr_auto] gap-2 border-b border-neutral-950/10 p-3">
        <Select aria-label="페르소나 생성 방식" value={world.run.config.experiment.personaMode} onChange={(event) => onPersonaMode(event.target.value)}>
      ...
      <div className="grid gap-3 bg-neutral-50 p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <p className="text-sm tabular-nums text-neutral-500">SEED {world.run.config.experiment.agentSeed}</p>
        </div>
          ...
          {Object.entries(selected.persona).map(([trait, score]) => (
            <div key={trait} className="grid grid-cols-[3rem_minmax(0,1fr)_1.5rem] items-center gap-2 text-sm text-neutral-600">
              ...
              <span className="h-1.5 overflow-hidden rounded-full bg-neutral-200">
                <span className="h-full rounded-full bg-neutral-800" style={{ width: `${Number(score) * 10}%` }} />
              </span>
              <span className="text-right tabular-nums font-medium text-neutral-800">{String(score)}</span>
            </div>
          ))}
        </div>
      </div>
