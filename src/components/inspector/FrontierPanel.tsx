import { LAUNCH_CONFIG, RESOURCE_CONFIG } from '../../config/simulation.ts'
...
function BuildingProgress({ building, storage }) {
  ...
  const inputProgress = inputs.length
    ? inputs.reduce((sum, [resource, amount]) => sum + Math.min(storage.resources[resource] / Number(amount), 1), 0) / inputs.length
    : 1
        ...
        <div className="divide-y divide-neutral-950/10">
          {frontier.map((building) => (
            <BuildingProgress
              key={building.id}
              building={building}
              storage={world.storages.find((storage) => storage.id === building.storageId) ?? world.storages[0]}
            />
          ))}
        </div>
