import { RESOURCE_CONFIG, SIMULATION_CONFIG } from '../../config/simulation.ts'
import { cn } from '../../lib/cn.ts'
import { distance, footprintCells } from '../../simulation/engine.ts'
...
export function WorldMap({ className = '', world, displayedPositions, selectedAgentId, observationMode, onSelectAgent }) {
  ...
  const buildings = world.buildings.filter((building) => building.position)

  return (
    <div className={cn('overflow-auto rounded-lg bg-white shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_2px_6px_rgba(0,0,0,0.05)]', className)}>
      <div className="world-map relative aspect-square min-w-184 overflow-hidden bg-white" aria-label={`${SIMULATION_CONFIG.width} 곱하기 ${SIMULATION_CONFIG.height} 시뮬레이션 지도`}>
